...
class ArticleController extends MobileBaseController
{
    public function index()
    {
        ...
        $commentList=M('distribution_comment')->where($commenWhere)->order('addtime DESC')->limit('5')->select();
 ...
 */		
	...
	public function readChapterFromBookCase()
	{
		...
		$book=M('article_article')->find($bookCaseInfo['articleid']);
		if($book){
	...
	public function getUserReadChapterCount($chapterorder)
	{
		...
		if($weiXinBrower)
		{
			...
			if(!($_SESSION['subscribe'])){
				...
				if(cookie('fromid')==''){
					$channel = M('distribution_channels')->where(array('channelid' => $user['channelid']))->find();
					//获取fromid
					if ($channel['pid'] == 0) {
		...
		if($weiXinBrower)
		{
			...
			else
			{
				...
				if($count)
				{
					...
					$nextChapaterInfoList      = M('article_chapter')->where($condition)->limit(1)->select();
            ...
            if(!$channel['adpower']||$channel['adplan']==0){
                ...
                if($ad['adtype']==1){
                    if($ad['picurl']==''&&substr($ad['url'], 0, strlen('http')) !== 'http'){
                        ...
                        $ad['picurl']='http://channel.mianfeidushu.com'.$ad['picurl'];
                    }
