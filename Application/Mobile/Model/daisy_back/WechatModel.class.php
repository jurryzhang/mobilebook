...
class WechatModel extends Model{
    ...
    private function _doSubscribe($postObj){
		...
		$str = sprintf($this->tpl['text'],$postObj->FromUserName,$postObj->ToUserName,time(),"欢迎关注".$wxname." 点我：<a href='http://m.kyread.com/Article/weiXinUserReadChapter/cid/".$this->_channelid.".html'>☞继续阅读☜</a>");
     ...
     *@postObj:响应的消息对象
     **/
    private function _doText($postObj,$key=''){
        ...
        if(!empty( $keyword ))
        {
            ...
            if($booklist){
                foreach ($booklist as $k=>$v){
                    ...
                    $contentStr.="<a href='http://m.kyread.com/Article/index/id/".$v['articleid']."/cid/".$this->_channelid.".html'>【点击查看】</a>\n";
                }
            }elseif ( $keyword=='kefu'){
                $contentStr ='请加客服微信:jiguangsunshine';
            }else{
    ...
    public function _dokeyWord($postObj,$key=''){
        ...
        $where=array('channelid'=>$this->_channelid,
                        'keyword'=>$keyword
            );
        ...
        if($info){
            ...
            $pic=$pic[0]['pic'];
            $pic='http://channel.mianfeidushu.com'.$pic;
